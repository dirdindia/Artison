...
export default function ArtworkModal({ isOpen, onClose, product, onSuccess, isViewMode = false }) {
  ...
  const fetchCategoriesAndSubCategorys = async () => {
    try {
      ...
      toast.error('Error', 'Failed to fetch categories or subCategories');
    }
  };
  ...
  const handleSingleImageUpload = async (e) => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        toast.success('Image Uploaded', 'Main display image successfully uploaded');
      }
    } catch (error) {
      toast.error('Upload Failed', error.response?.data?.message || 'Failed to upload image');
    } finally {
  ...
  const handleGalleryUpload = async (e) => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        toast.success('Images Uploaded', 'Gallery images successfully uploaded');
      }
    } catch (error) {
      toast.error('Upload Failed', error.response?.data?.message || 'Failed to upload gallery images');
    } finally {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.name || !formData.price || !formData.category) {
      return toast.error('Validation Error', 'Name, Price, and Category are required');
    }
    ...
    try {
      ...
      if (product) {
        await api.put(`/products/artist/${product._id}`, payload);
        toast.success('Artwork updated successfully');
      } else {
        await api.post('/products/artist', payload);
        toast.success('Artwork created successfully');
      }
      onSuccess();
      onClose();
    } catch (error) {
      toast.error('Error', error.response?.data?.message || 'Failed to Submit Artwork');
    } finally {
      setLoading(false);
    }
  };
        ...
        <div className="p-4 border-t border-[#eae0d5] bg-[#fdfbf7] flex justify-between items-center shrink-0">
          ...
          <div className="flex gap-3">
            ...
            {!isViewMode && (
              ...
              >
                {loading ? <Loader2 className="w-4 h-4 animate-spin" /> : 'Submit Artwork'}
              </button>
            )}
          </div>
        </div>
