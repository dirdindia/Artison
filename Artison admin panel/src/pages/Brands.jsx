...
const mockBrands = [
  { id: 1, name: 'Artisan Originals', slug: 'artisan-originals', products: 45, status: 'Active' },
  { id: 2, name: 'Studio Palette', slug: 'studio-palette', products: 18, status: 'Active' },
  { id: 3, name: 'Vintage Craft', slug: 'vintage-craft', products: 5, status: 'Inactive' },
];
    ...
    <div className="max-w-6xl mx-auto space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-[#5a4d4d] mt-1">Manage artists, studios, or collections as brands.</p>
        </div>
        <button 
          ...
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-[#eae0d5]">
              {mockBrands.map((brand) => (
                <tr key={brand.id} className="hover:bg-[#fdfbf7]/50 transition-colors cursor-pointer group">
                  <td className="px-6 py-4 font-medium text-[#3b2f2f]">{brand.name}</td>
                  <td className="px-6 py-4 text-[#5a4d4d]">{brand.slug}</td>
                  <td className="px-6 py-4 text-[#5a4d4d]">{brand.products}</td>
                  <td className="px-6 py-4">
                    <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
                      brand.status === 'Active' ? 'bg-emerald-100 text-emerald-800' : 'bg-gray-100 text-gray-800'
                    }`}>
                      {brand.status}
                    </span>
                  </td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-2xl overflow-hidden flex flex-col max-h-[90vh]">
            <div className="p-6 border-b border-[#eae0d5] flex items-center justify-between bg-[#fdfbf7]">
              <h2 className="text-xl font-bold text-[#3b2f2f]">Add New Brand</h2>
              <button 
              ...
              <form className="space-y-5">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                  <div className="space-y-1.5">
                    <label htmlFor="name" className="block text-sm font-medium text-[#5a4d4d]">Brand Name</label>
                    <input 
                      type="text" 
                      id="name" 
                ...
                <div className="space-y-1.5">
                  ...
                  <textarea 
                    id="description" 
                    rows={3}
                    placeholder="Describe the brand or artist's style..."
                ...
                <div className="space-y-1.5">
                  <label className="block text-sm font-medium text-[#5a4d4d]">Brand Logo</label>
