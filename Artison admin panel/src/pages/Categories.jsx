...
const Categories = () => {
  ...
  const fetchCategories = async (currentPage = 1) => {
    ...
    try {
      ...
      Alert.error('Error', 'Failed to fetch categories');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="max-w-7xl mx-auto">
      <div className="flex justify-between items-center mb-6">
        <div>
          ...
          <p className="text-[#5a4d4d] text-sm mt-1">Manage your product categories ({totalItems} total)</p>
        </div>
        <button 
          ...
          <table className="w-full text-left text-sm text-[#5a4d4d]">
            ...
            <tbody>
              {loading ? (
                ...
                categories.map((cat) => (
                  <tr key={cat._id} className="border-b border-[#eae0d5]/50 hover:bg-[#fdfbf7]/50 transition-colors">
                    <td className="px-6 py-4 font-medium text-[#3b2f2f] flex items-center gap-3">
                      {cat.image ? (
                        <button 
                          type="button" 
                          onClick={() => setViewingImage(cat.image)} 
                    ...
                    <td className="px-6 py-4">
                      <span className={`px-2.5 py-1 rounded-full text-xs font-medium ${cat.isActive ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
                        {cat.isActive ? 'Active' : 'Inactive'}
                      </span>
                    </td>
                    <td className="px-6 py-4 text-right">
                      <div className="flex justify-end gap-2">
                        <button onClick={() => openViewModal(cat)} className="p-2 text-gray-600 hover:bg-gray-100 rounded-lg transition-colors cursor-pointer">
