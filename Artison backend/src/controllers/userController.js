...
const getAllArtists = async (req, res) => {
  try {
    ...
    const users = await User.find({ role: 'artist' })
      .select('-password')
      .populate('artCategory', 'name')
      .populate('artSubcategory', 'name')
      .sort({ createdAt: -1 })
      .skip(skip)
      .limit(limit);
...
// @desc    Toggle artist approval status
// @route   PUT /api/users/artists/:id/approve
// @access  Private/Admin
const toggleArtistApproval = async (req, res) => {
  try {
    ...
    if (user && user.role === 'artist') {
      ...
      res.json({
        success: true,
        message: `Artist ${user.isApproved ? 'approved' : 'unapproved'} successfully`,
        data: user,
      });
    } else {
